...
class StreamingLogAnalyzer:
    ...
    def _compile_patterns(self) -> Dict:
        ...
        return {
            ...
            'gpt4_ref': re.compile(r'gpt-4', re.IGNORECASE),
            ...
            'test_summary': re.compile(r'=.*?(\d+)\s+failed.*?(\d+)\s+passed.*?(\d+)\s+skipped.*?(\d+)\s+errors', re.IGNORECASE),
        }
    ...
    def process_chunk(self, lines: List[str]) -> None:
        ...
        for line in lines:
            ...
            if self.patterns['gpt4_ref'].search(line):
            ...
            with open(self.log_path, 'r', encoding='utf-8', errors='ignore') as f:
                for line in f:
                    ...
                    if len(chunk) >= self.chunk_size:
                        ...
                        if chunk_count % 10 == 0:
                            print(f"   Processados: {chunk_count * self.chunk_size:,} linhas...", end='\r')
...
def extract_key_sections(log_path: str, output_dir: str) -> Dict[str, str]:
    ...
    print(f"📋 Extraindo seções-chave do log...")
...
def main():
    ...
    parser.add_argument('--output-dir', default='data/test_reports/analysis', help='Diretório de saída')
    ...
    if args.extract_sections:
        sections = extract_key_sections(str(log_path), str(output_dir / 'sections'))
