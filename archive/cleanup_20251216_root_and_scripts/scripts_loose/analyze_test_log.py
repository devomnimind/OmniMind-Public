...
class LogAnalyzer:
    ...
    def _extract_critical_issues(self, lines: List[str]):
        ...
        critical_patterns = [
            ...
            (r'timeout.*30\s*(?:s|sec)', 'Timeout de 30s detectado (deveria ser 800s para testes reais)'),
...
def main():
    ...
    log_path = sys.argv[1] if len(sys.argv) > 1 else 'data/test_reports/consolidated_fast_20251207_120233.log'

    analyzer = LogAnalyzer(log_path)
    report = analyzer.analyze()

    # Salvar relatório JSON
    output_path = Path(log_path).parent / f"analysis_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
