...
class BrowserVerifier:
    ...
    def verify_web_app(
        ...
        timestamp = time.time()
        screenshot_path = str(
            self.screenshots_dir / f"screenshot_{int(timestamp)}.png"
        )
        ...
        logger.info(
            f"Browser verification: url={url}, passed={passed}"
        )
        ...
        for pattern, weight in top_patterns:
            if weight > 0.5:
                recommendations.append(
                    f"Continue usando abordagem: {pattern}"
                )
            elif weight < -0.5:
                recommendations.append(
                    f"Evite abordagem: {pattern}"
                )
...
class ModelSelector:
    ...
    def select_model(
        ...
        logger.info(
            f"Model selected: {selected_model.value} "
...
class AgenticIDE:
    ...
    def __init__(
        ...
        logger.info(
            f"Agentic IDE initialized: workspace={workspace_path}"
        )
    ...
    def create_task(
        ...
        logger.info(
            f"Task created: {task_id} for agent={agent_id}, "
            f"model={model.value}"
        )
    ...
    def verify_artifact(
        ...
        if verification_type == "browser":
            ...
            verification = self.browser_verifier.verify_web_app(
                ...
                expected_elements=[".app-container"]
            )
    ...
    def get_insights(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'verified_artifacts': sum(
                1 for a in self.artifacts if a.verified
            ),
            'self_improvement': self.self_improvement.get_learned_insights(),
            'model_usage': {
                model.value: sum(
                    1 for entry in self.model_selector.usage_history
