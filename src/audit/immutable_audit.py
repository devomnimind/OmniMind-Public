...
class ImmutableAuditSystem:
    ...
    def __init__(self, log_dir: str = "~/projects/omnimind/logs"):
        ...
        # 🔧 LAZY LOADING: Cache de verificação para performance
    ...
    def _auto_recover_chain(self) -> None:
        ...
        # Verificar integridade atual (sem cache para forçar nova verificação)
        integrity_check = self.verify_chain_integrity(use_cache=False)
    ...
    def verify_chain_integrity(self, use_cache: bool = True) -> Dict[str, Any]:
        """
        Verifica integridade completa da cadeia de hash com cache inteligente.
        Permite quebras controladas na cadeia quando há reinicializações do sistema.

        Args:
            use_cache: Se True, usa cache para evitar verificações desnecessárias

        Returns:
            Dicionário com resultado da verificação
        """
        # Verificar cache primeiro
        if use_cache and self._integrity_verified:
            cache_age = time.time() - self._cache_timestamp
            if cache_age < 3600:  # Cache válido por 1 hora
                cached_result = self._verification_cache.get("result")
                if cached_result:
                    return cached_result  # type: ignore

        # Executar verificação real
        result = self._do_verify_chain_integrity()

        # Atualizar cache
        if use_cache:
            self._integrity_verified = True
            self._cache_timestamp = time.time()
            self._verification_cache = {"result": result}

        return result

    def _do_verify_chain_integrity(self) -> Dict[str, Any]:
        """
        Implementação real da verificação de integridade (sem cache).
        Permite quebras controladas na cadeia quando há reinicializações do sistema.
