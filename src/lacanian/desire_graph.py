...
class SignifierChain:
    ...
    def metaphoric_substitution(
        ...
        """
        new_chain = [
            new_signifier if s == old_signifier else s
            for s in self.chain
        ]
        
        return SignifierChain(
            chain=new_chain,
            quilting_points=self.quilting_points.copy()
        )
...
class LacanianGraphII:
    ...
    def add_signifier(
        ...
        signifier = Signifier(
            symbol=symbol,
            position=position,
            jouissance_intensity=jouissance
        )
    ...
    def compute_desire(self) -> Dict[str, Any]:
        ...
        jouissance = sum(
            self.jouissance_map.get(c, 0.0)
            for c in unsatisfied_connections
    ...
    def fantasy_formula(self) -> Tuple[str, str]:
        ...
        object_a_candidates = [
            s for s, sig in self.signifiers.items()
...
class DesireGraphArchitecture:
    ...
    def process_desire(
        ...
        repetition = self.jouissance_system.beyond_pleasure_principle(
            action_history
        )
