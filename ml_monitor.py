...
class MLMonitor:
    ...
    def generate_report(self) -> dict:
        ...
        if len(usage_history) >= 2:
            ...
            github_trend = recent.get('github_requests_remaining', 0) - previous.get('github_requests_remaining', 0)
        ...
        return {
            ...
            "trends": {
                "github_requests_trend": github_trend,
    ...
    try:
        while True:
            ...
            if int(time.time()) % 600 < 30:  # aproximadamente a cada 10 min
                ...
                print("\\n📈 RELATÓRIO PERIÓDICO:")
                print(f"   GitHub requests restantes: {report['current_usage']['github_usage']['requests_remaining']}")
                print(f"   Custo estimado: ${report['current_usage']['github_usage']['estimated_cost']:.3f}")
