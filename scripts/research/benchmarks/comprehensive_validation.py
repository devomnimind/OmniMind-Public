...
class ComprehensiveValidator:
    ...
    def run_paper2_validation(self):
        ...
        if self.ibm_time_used >= IBM_QUANTUM_TIME_REMAINING:
            ...
            results_paper2["quantum_budget_status"] = (
                f"{IBM_QUANTUM_TIME_REMAINING - self.ibm_time_used}s remaining"
            )
    ...
    def generate_report(self):
        ...
        # Save JSON
        output_file = (
            self.output_dir / f"validation_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
