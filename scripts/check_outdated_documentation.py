...
def find_test_count_in_files(
    project_root: Path, extensions: List[str] = [".md", ".txt", ".rst"]
) -> List[Dict]:
...
def main():
    ...
    if actual_stats:
        print(
            f"Total de arquivos de teste: {Colors.OKGREEN}{actual_stats.get('total_test_files', 'N/A')}{Colors.ENDC}"
        )
        print(
            f"Total de funções de teste definidas: {Colors.OKGREEN}{actual_stats.get('total_test_functions_defined', 'N/A')}{Colors.ENDC}"
        )
        print(
            f"Testes executáveis: {Colors.OKGREEN}{actual_stats.get('expected_runnable_tests', 'N/A')}{Colors.ENDC}"
        )
        print(
