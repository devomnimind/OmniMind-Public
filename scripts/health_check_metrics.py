...
class MetricsHealthChecker:
    ...
    def __init__(
        ...
        self.expected_metrics = {
            ...
            "rhizome": ["flows_per_cycle", "average_intensity", "source_diversity", "flow_rate"],
        }
    ...
    def check_module_health(
        self, module_name: str, metrics: Dict[str, Any], timestamp: float
    ) -> ModuleHealth:
        """
    ...
    def check_system_health(self, metrics_data: Dict[str, Any]) -> SystemHealth:
        ...
        if phi_value and phi_value < 0.01:
            recommendations.append("Φ muito baixo - considerar aumentar integração entre módulos")
        ...
        if critical_modules > 0:
            ...
            recommendations.append(
                f"Módulos críticos: {', '.join(critical_names)} - " f"requer atenção imediata"
            )
...
def main():
    ...
    parser.add_argument(
        "--phi-min", type=float, default=0.001, help="Valor mínimo aceitável de Φ (nats)"
    )
    parser.add_argument(
        "--phi-max", type=float, default=1.0, help="Valor máximo aceitável de Φ (nats)"
    )
    parser.add_argument(
        "--silent-threshold",
        type=int,
    ...
    checker = MetricsHealthChecker(
        phi_min=args.phi_min, phi_max=args.phi_max, silent_threshold_minutes=args.silent_threshold
    )
    ...
    print(
        f"Módulos: {health.total_modules} total, "
    ...
    for module in health.modules:
        status_icon = {"healthy": "✓", "warning": "⚠", "critical": "✗", "silent": "○"}.get(
            module.status, "?"
        )
    ...
    exit_code = {"healthy": 0, "degraded": 1, "critical": 2}.get(health.status, 3)
