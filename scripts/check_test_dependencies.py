...
def main():
    ...
    for module in missing:
        ...
        if 'system_deps' in info:
            print(f"    {Colors.WARNING}Requer deps do sistema: {', '.join(info['system_deps'])}{Colors.ENDC}")
    ...
    print(f"  Percentual bloqueado: {Colors.FAIL}{total_blocked/2412*100:.1f}%{Colors.ENDC} dos 2412 testes")
    ...
    if choice == '1':
        ...
        selections = input(f"\n{Colors.BOLD}Digite os números separados por vírgula: {Colors.ENDC}").strip()
        ...
        print(f"{Colors.WARNING}Nota: Isso instalará TODAS as dependências do projeto.{Colors.ENDC}")
