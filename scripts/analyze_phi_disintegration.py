...
def analyze_phi_causal_vs_workspace(metrics: Dict) -> Dict:
    ...
    analysis = {
        ...
        "ratio_avg": (
            np.mean(phi_causal) / np.mean(phi_workspace) if np.mean(phi_workspace) > 0 else 0.0
        ),
    }
...
def identify_degradation_pattern(metrics: Dict) -> Dict:
    ...
    return {
        ...
        "degradation_percentage": (
            ((max_phi - phi_progression[-1]) / max_phi * 100) if max_phi > 0 else 0.0
        ),
...
def main():
    ...
    if "error" not in causal_analysis:
        ...
        print(
            f"Φ workspace: {causal_analysis['phi_workspace']['mean']:.6f} ± {causal_analysis['phi_workspace']['std']:.6f}"
        )
        print(
            f"  Range: [{causal_analysis['phi_workspace']['min']:.6f}, {causal_analysis['phi_workspace']['max']:.6f}]"
        )
        print(
            f"Φ causal (RNN): {causal_analysis['phi_causal']['mean']:.6f} ± {causal_analysis['phi_causal']['std']:.6f}"
        )
        print(
            f"  Range: [{causal_analysis['phi_causal']['min']:.6f}, {causal_analysis['phi_causal']['max']:.6f}]"
        )
        ...
        if causal_analysis["ratio_avg"] > 10:
            print(
                f"⚠️  DESACOPLAMENTO: Φ causal é {causal_analysis['ratio_avg']:.1f}x maior que Φ workspace"
            )
    ...
    if "error" not in causal_analysis:
        if abs(causal_analysis["correlation"]) < 0.1:
            issues.append(
                "🔴 DESACOPLAMENTO RNN-WORKSPACE: Φ causal não está sendo integrado no cálculo de Φ workspace"
            )
        if causal_analysis["ratio_avg"] > 10:
            issues.append(
                f"🔴 DESACOPLAMENTO: Φ causal ({causal_analysis['phi_causal']['mean']:.3f}) >> Φ workspace ({causal_analysis['phi_workspace']['mean']:.3f})"
            )
    ...
    if "error" not in degradation:
        if degradation["degradation_percentage"] > 50:
            issues.append(
                f"🔴 DEGRADAÇÃO SEVERA: {degradation['degradation_percentage']:.1f}% de queda após pico"
            )
        if degradation["degradation_rate_per_cycle"] < -0.0005:
            issues.append(
                f"🔴 TAXA DE DEGRADAÇÃO ALTA: {degradation['degradation_rate_per_cycle']:.6f} por ciclo"
            )
