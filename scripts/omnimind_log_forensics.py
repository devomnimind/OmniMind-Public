...
PATTERNS = {
    ...
    "force_metric": re.compile(
        r"força[_\s]*[:=]\s*([\d.]+)|force[_\s]*[:=]\s*([\d.]+)", re.IGNORECASE
    ),
    ...
    "structural_failure": re.compile(
        r"Structural Failure|Falha estrutural|divergência=([\d.]+)", re.IGNORECASE
    ),
    ...
    "cuda_oom": re.compile(
        r"CUDA out of memory.*?Tried to allocate ([\d.]+)\s*(\w+)", re.IGNORECASE
    ),
    ...
    "metacognition_action_failed": re.compile(
        r"meta.*cogn.*action.*failed|metacognition.*action.*failed", re.IGNORECASE
    ),
            ...
            with open(self.log_path, "r", encoding="utf-8", errors="replace") as f:
                ...
                for line in f:
                    ...
                    if len(chunk) >= self.chunk_size:
                        ...
                        if chunk_count % 10 == 0:
                            print(
                                f"   ...processadas {self.stats['lines_processed']:,} linhas...",
                                end="\r",
                            )
    ...
    def _generate_report(self) -> Dict:
        ...
        report = {
            ...
            "insufficient_history": {
                ...
                "numeric_patterns": list(set(self.insufficient_history_numeric))[
                    :20
                ],  # Únicos, limitado a 20
        ...
        # Salvar JSON
        output_file = (
            f"data/test_reports/analysis/forensics_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
    ...
    def _print_report(self, report: Dict, output_file: str) -> None:
        ...
        if total > 0:
            ...
            if tests.get("duration", 0) > 0:
                print(
                    f"   • ⏱️  Duração:    {tests['duration']:.1f}s ({tests['duration']/60:.1f} min)"
                )
        print()
        ...
        if insufficient_info.get("total_count", 0) > 0:
            ...
            if insufficient_info.get("numeric_patterns"):
                print(
                    f"      • Padrões numéricos detectados: {', '.join(insufficient_info['numeric_patterns'][:10])}"
                )
...
Exemplos:
  ...
  python scripts/omnimind_log_forensics.py --compare report1.json report2.json
    ...
    parser.add_argument(
        "--compare", nargs=2, metavar=("REPORT1", "REPORT2"), help="Comparar dois relatórios JSON"
    )
    ...
    if args.compare:
        ...
        output_file = (
            f"data/test_reports/analysis/comparison_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
        Path(output_file).parent.mkdir(parents=True, exist_ok=True)
        ...
        for metric, changes in comparison["consciousness_changes"].items():
            print(
                f"   {metric}: {changes['before']:.4f} → {changes['after']:.4f} "
