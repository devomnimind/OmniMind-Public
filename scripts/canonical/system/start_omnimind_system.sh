...
# Descrição: Retorna timestamp formatado para logs e registros de sistema
# Uso: echo "[$(get_timestamp)] Mensagem de log"
# ============================================================================
get_timestamp() {
    date '+%Y-%m-%d %H:%M:%S'
}
...
echo ""
echo -e "${GREEN}🟢 SISTEMA PRONTO PARA OPERAÇÃO CONTÍNUA${NC}"

# ============================================================================
# MANUTENÇÃO: Escrever PID e aguardar indefinidamente
# ============================================================================
# Para systemd TYPE=forking: escrever nosso PID (do script de inicialização)
# e depois aguardar indefinidamente. systemd monitorará os child processes.
echo $$ > /var/run/omnimind.pid 2>/dev/null || true

# Criar diretório de logs se não existir
mkdir -p logs 2>/dev/null || true

# Registrar reinicialização no arquivo de startup log
echo "[$(get_timestamp)] ✅ Sistema OmniMind reiniciado com sucesso. Aguardando continuamente..." >> logs/startup.log 2>/dev/null || true
echo "[$(get_timestamp)] PIDs do Cluster:" >> logs/startup.log 2>/dev/null || true
echo "[$(get_timestamp)]   - Backend Cluster Master: $$" >> logs/startup.log 2>/dev/null || true

# Manter o processo pai vivo indefinidamente (systemd monitorará os filhos)
# Isso evita que o script saia e systemd cancele todos os processos
tail -f /dev/null &
TAIL_PID=$!
