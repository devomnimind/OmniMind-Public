...
def find_source_modules_without_tests(src_dir: Path, tests_dir: Path) -> List[Dict[str, any]]:
    ...
    for src_file in src_dir.rglob('*.py'):
        ...
        if any(pattern in str(src_file) for pattern in [
...
def main():
    ...
    print(f"  Arquivos com erros de importação: {Colors.FAIL if import_errors else Colors.OKGREEN}{len(import_errors)}{Colors.ENDC}")
    print(f"  Arquivos com marcadores skip: {Colors.WARNING if files_with_skips else Colors.OKGREEN}{files_with_skips}{Colors.ENDC}")
    ...
    print(f"  Testes síncronos: {Colors.OKBLUE}{total_test_functions - total_async_tests}{Colors.ENDC}")
    ...
    print(f"  Total estimado pulados: {Colors.WARNING}{total_skipped + total_skipif + total_xfail}{Colors.ENDC}")
    ...
    print(f"  Testes executáveis (sem erros import): {Colors.OKGREEN}{actually_runnable}{Colors.ENDC}")
    print(f"  Testes bloqueados por import: {Colors.FAIL}{blocked_by_imports}{Colors.ENDC}")
    print(f"  Testes marcados para skip: {Colors.WARNING}{total_skipped + total_skipif + total_xfail}{Colors.ENDC}")
    ...
    print(f"Testes bloqueados (imports):    {Colors.FAIL}{blocked_by_imports:5d}{Colors.ENDC} ({blocked_by_imports/total_test_functions*100:.1f}%)")
    print(f"Testes marcados skip:           {Colors.WARNING}{total_skipped + total_skipif:5d}{Colors.ENDC} ({(total_skipped + total_skipif)/total_test_functions*100:.1f}%)")
    print(f"{'─'*40}")
    print(f"Testes esperados na execução:   {Colors.OKGREEN}{actually_runnable:5d}{Colors.ENDC} ({actually_runnable/total_test_functions*100:.1f}%)")
    ...
    if untested:
        ...
        print(f"Módulos críticos sem testes: {Colors.FAIL if critical_untested else Colors.OKGREEN}{len(critical_untested)}{Colors.ENDC}")
