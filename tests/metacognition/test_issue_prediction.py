...
class TestIssuePredictionEngine:
    ...
    def test_predict_resource_exhaustion_warning(self):
        ...
        cpu_predictions = [
            ...
            if p.metric_type == MetricType.CPU_USAGE
        ]
        
        if cpu_predictions:
            pred = cpu_predictions[0]
            assert pred.severity in [PredictionSeverity.WARNING, PredictionSeverity.CRITICAL]
    ...
    def test_predict_anomaly(self):
        ...
        error_predictions = [
            p for p in predictions
            if p.metric_type == MetricType.ERROR_RATE
        ]
    ...
    def test_predict_performance_degradation(self):
        ...
        perf_predictions = [
            p for p in predictions
            if p.metric_type == MetricType.RESPONSE_TIME
        ]
