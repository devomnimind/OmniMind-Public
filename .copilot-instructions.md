...
### 7. File Organization Discipline (CRITICAL)

**MANDATORY FILE ORGANIZATION RULES:**

- **📁 Documentation:** ALL documentation files (`.md`, `.txt`, `.rst`) MUST go in `docs/` subdirectory
  - Use subdirectories like `docs/reports/`, `docs/guides/`, `docs/api/` for organization
  - NEVER create documentation files directly in project root

- **🔧 Scripts:** ALL executable scripts (`.sh`, `.py` for utilities) MUST go in `scripts/` subdirectory
  - Use descriptive names and organize by purpose
  - NEVER leave utility scripts in project root

- **⚙️ Configuration:** Configuration files go in appropriate subdirectories:
  - Python configs: `config/` or project root if essential for setup
  - CI/CD configs: `.github/`
  - Linting/formatting configs: project root (`.flake8`, `mypy.ini`, etc.)

- **📊 Data & Logs:** 
  - Application data: `data/` subdirectory
  - Temporary files: `temp/` subdirectory  
  - Logs: `logs/` subdirectory
  - NEVER store data/logs in project root

- **🏗️ Build Artifacts:** 
  - Go in `build/`, `dist/`, or `target/` directories
  - NEVER leave build artifacts in project root

**PROJECT ROOT CONTENT POLICY:**
- ✅ Essential setup files: `README.md`, `requirements*.txt`, `setup.py`, `pyproject.toml`
- ✅ Core configuration: `.gitignore`, `.env.template`, linting configs
- ✅ Hidden/dotfiles: `.github/`, `.vscode/`, `.omnimind/`
- ❌ Documentation files
- ❌ Utility scripts
- ❌ Data files
- ❌ Build artifacts
...
1. **Search for references:** Use `grep` to find all references to file paths
2. **Check imports:** Verify Python imports still work
3. **Update documentation:** Update any docs referencing moved files
4. **Test functionality:** Run relevant tests to ensure nothing broke
5. **Update scripts:** Modify any scripts that reference moved files
...
9. **Test base= 90% coverage:** New code must meet this standard
